...
def parse_args():
    ...
    parser.add_argument('--offload', type=bool, default=True, help='')
    # deepspeed features
...
def main():
    ...
    if args.train_type == "lora":
        model = MODE[args.mode]["model"].from_pretrained(args.model_name_or_path, trust_remote_code=True,torch_dtype=torch.float16)
        ...
        config = MODE[args.mode]["config"].from_pretrained(args.model_name_or_path, trust_remote_code=True,torch_dtype=torch.float16)
        config.pre_seq_len = args.pre_seq_len
        config.prefix_projection = args.prefix_projection
        model = MODE[args.mode]["model"].from_pretrained(args.model_name_or_path, config=config, trust_remote_code=True,torch_dtype=torch.float16)
        for name, param in model.named_parameters():
    ...
    AdamOptimizer =  FusedAdam if args.offload else DeepSpeedCPUAdam
