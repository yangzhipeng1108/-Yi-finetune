...
deepspeed --include localhost:0,1,2,3,4,5,6,7 --master_port 29000 train.py \
                ...
                --freeze_module_name "layers.39.,layers.38.,layers.37.,layers.36." \
                --seed 1234 \
                --ds_file ds_zero2_no_offload.json \
                --gradient_checkpointing \
                --show_loss_step 10 \
...
deepspeed --hostfile  hostfile  train.py \
                ...
                --model_name_or_path /root/llama1/rl_dpo/Baichuan2-13B-Chat \
                --per_device_train_batch_size 1 \
                --max_len 1560 \
                --max_src_len 1024 \
                --learning_rate 1e-4 \
                ...
                --mode baichuan2 \
                --train_type freeze \
                --freeze_module_name "layers.39.,layers.38.,layers.37.,layers.36.,layers.35.,layers.34.,layers.33.,layers.32.,layers.31.,layers.30.,layers.29.,layers.28" \
                --seed 1234 \
                --ds_file ds_zero3++.json  \
                --gradient_checkpointing \
                --show_loss_step 10 \
                --output_dir ./output-glm2
